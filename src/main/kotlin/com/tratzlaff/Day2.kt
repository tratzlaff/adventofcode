...
fun main() {
    ...
    """.trimIndent()
    println("Part 1: "+day.solvePart1(input)) // 2101
    println("Part 2: "+day.solvePart2(input)) // 58269
}
...
class Day2 {
    ...
    fun solvePart1(input: String): Int {
        ...
        return games.filter { it.isPossible(12, 13, 14) }.sumOf { it.id }
    }
    ...
    data class Game(val id: Int, val rounds: List<Map<String, Int>>) {
        fun getPower(): Int {
            val maxRed = rounds.maxOfOrNull { it.getOrDefault("red", 0) } ?: 0
            val maxGreen = rounds.maxOfOrNull { it.getOrDefault("green", 0) } ?: 0
            val maxBlue = rounds.maxOfOrNull { it.getOrDefault("blue", 0) } ?: 0
    ...
    private fun parseGames(input: String): List<Game> {
        return input.lines().map { line ->
            ...
            val id = parts[0].removePrefix("Game ").trim().toInt()
            val rounds = parts[1].split(";").map { round ->
                round.trim().split(",").associate {
    ...
    private fun Game.isPossible(redMax: Int = 0, greenMax: Int = 0, blueMax: Int = 0): Boolean {
        return rounds.all { round ->
            round.getOrDefault("red", 0) <= redMax &&
                    round.getOrDefault("green", 0) <= greenMax &&
                    round.getOrDefault("blue", 0) <= blueMax
        }
    }
}